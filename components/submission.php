...
class Kiwi_CF_Submission {
	...
	public static function get_instance( KiwiCfContactForm $contact_form = null, $args = '' ) {
	...
	private function setup_posted_data() {
		...
		foreach ( (array) $tags as $tag ) {
			...
			if ( KIWI_CF_USE_PIPE
			and $pipes instanceof Kiwi_CF_Pipes
			and ! $pipes->zero() ) {
			...
			$value = apply_filters( "kiwi_cf_posted_data_{$type}", $value,
				$value_orig, $tag );
		...
		$this->posted_data = apply_filters( 'kiwi_cf_posted_data', $posted_data );
		...
		if ( ! $this->validate() ) { // Validation error occured
			...
			if ( '' === $this->get_response() ) {
				$this->set_response( $contact_form->filter_message(
					__( "Sending mail has been aborted.", 'kiwi-contact-form' ) )
				);
			}
			...
			do_action( 'kiwi_cf_mail_sent', $contact_form );

		} else {
			$this->set_status( 'mail_failed' );
			$this->set_response( $contact_form->message( 'mail_sent_ng' ) );

			do_action( 'kiwi_cf_mail_failed', $contact_form );
		}
	...
	private function get_remote_ip_addr() {
		...
		return apply_filters( 'kiwi_cf_remote_ip_addr', $ip_addr );
	}
	...
	private function validate() {
		...
		require_once KIWI_CF_PLUGIN_DIR . '/includes/validation.php';
		$result = new Kiwi_CF_Validation();

		$tags = $this->contact_form->scan_form_tags();

		foreach ( $tags as $tag ) {
			$type = $tag->type;
			$result = apply_filters( "kiwi_cf_validate_{$type}", $result, $tag );
		}

		$result = apply_filters( 'kiwi_cf_validate', $result, $tags );
	...
	private function accepted() {
		return apply_filters( 'kiwi_cf_acceptance', true, $this );
	}
	...
	private function spam() {
		...
		and current_user_can( 'kiwi_cf_submit', $this->contact_form->id() ) ) {
			return $spam;
		}
		...
		if ( strlen( $user_agent ) < 2 ) {
			...
			$this->add_spam_log( array(
				'agent' => 'kiwi',
				'reason' => __( "User-Agent string is unnaturally short.", 'kiwi-contact-form' ),
			) );
		}
		...
		if ( ! $this->verify_nonce() ) {
			...
			$this->add_spam_log( array(
				'agent' => 'kiwi',
				'reason' => __( "Submitted nonce is invalid.", 'kiwi-contact-form' ),
			) );
		}
		...
		if ( $this->is_blacklisted() ) {
			...
			$this->add_spam_log( array(
				'agent' => 'kiwi',
				'reason' => __( "Blacklisted words are used.", 'kiwi-contact-form' ),
			) );
		}

		return apply_filters( 'kiwi_cf_spam', $spam );
	}
	...
	private function verify_nonce() {
		...
		return kiwi_cf_verify_nonce( $_POST['_wpnonce'] );
	}

	private function is_blacklisted() {
		$target = kiwi_cf_array_flatten( $this->posted_data );
		$target[] = $this->get_meta( 'remote_ip' );
		$target[] = $this->get_meta( 'user_agent' );
		$target = implode( "\n", $target );

		return (bool) apply_filters( 'kiwi_cf_submission_is_blacklisted',
			kiwi_cf_blacklist_check( $target ), $this );
	}
	...
	private function before_send_mail() {
		...
		do_action_ref_array( 'kiwi_cf_before_send_mail', array(
			$this->contact_form,
			&$abort,
			$this,
		) );
	...
	private function mail() {
		...
		$skip_mail = apply_filters( 'kiwi_cf_skip_mail',
		...
		$result = Kiwi_CF_Mail::send( $contact_form->prop( 'mail' ), 'mail' );
		...
		if ( $result ) {
			...
			$additional_mail = apply_filters( 'kiwi_cf_additional_mail',
				$additional_mail, $contact_form );

			foreach ( $additional_mail as $name => $template ) {
				Kiwi_CF_Mail::send( $template, $name );
			}
