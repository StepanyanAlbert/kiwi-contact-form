...
add_action( 'kiwi_cf_init', 'kiwi_cf_constant_contact_register_service', 10, 0 );

function kiwi_cf_constant_contact_register_service() {
	$integration = KiwiCfIntegration::get_instance();

	$integration->add_category( 'email_marketing',
		__( 'Email Marketing', 'kiwi-contact-form' ) );

	$service = kiwi_CF_ConstantContact::get_instance();
	$integration->add_service( 'constant_contact', $service );
}

add_action( 'kiwi_cf_save_contact_form',
	'kiwi_cf_constant_contact_save_contact_form', 10, 1 );

function kiwi_cf_constant_contact_save_contact_form( $contact_form ) {
	$service = Kiwi_CF_ConstantContact::get_instance();
	...
	foreach ( $additional_settings as $setting ) {
		if ( preg_match_all( $pattern, $setting, $matches ) ) {
			foreach ( $matches[1] as $match ) {
				$name = trim( kiwi_cf_strip_quote( $match ) );
	...
	$key = sprintf( 'kiwi_cf_contact_form:%d', $contact_form->id() );

	$service->update_contact_lists( array( $key => $list_names ) );
}

add_action( 'kiwi_cf_submit', 'kiwi_cf_constant_contact_submit', 10, 2 );

function kiwi_cf_constant_contact_submit( $contact_form, $result ) {
	$service = Kiwi_CF_ConstantContact::get_instance();
	...
	$do_submit = apply_filters( 'kiwi_cf_constant_contact_submit',
	...
	$request_builder_class_name = apply_filters(
		'kiwi_cf_constant_contact_contact_post_request_builder',
		'kiwi_cf_ConstantContact_ContactPostRequest'
	);
...
if ( ! class_exists( 'Kiwi_CF_Service_OAuth2' ) ) {
	return;
}

class kiwi_CF_ConstantContact extends Kiwi_CF_Service_OAuth2 {
	...
	private function __construct() {
		...
		$option = (array) KIWICF::get_option( self::service_name );
		...
		add_action( 'kiwi_cf_admin_init', array( $this, 'auth_redirect' ) );
	}
	...
	public function auth_redirect() {
		...
		and current_user_can( 'kiwi_cf_manage_integration' ) ) {
			$redirect_to = add_query_arg(
				...
				menu_page_url( 'kiwi-integration', false )
			);
	...
	protected function save_data() {
		$option = array_merge(
			(array) KIWICF::get_option( self::service_name ),
			array(
        ...
        KIWICF::update_option( self::service_name, $option );
	}
	...
	public function get_title() {
		return __( 'Constant Contact', 'kiwi-contact-form' );
	}
	...
	protected function menu_page_url( $args = '' ) {
		...
		$url = menu_page_url( 'kiwi-integration', false );
	...
	public function load( $action = '' ) {
		...
		if ( 'setup' == $action and 'POST' == $_SERVER['REQUEST_METHOD'] ) {
			check_admin_referer( 'kiwi-constant-contact-setup' );
		...
		switch ( $message ) {
			case 'success':
				echo sprintf(
					...
					esc_html( __( "Connection established.", 'kiwi-contact-form' ) )
				);
				break;
			case 'failed':
				echo sprintf(
					'<div class="error notice notice-error is-dismissible"><p><strong>%1$s</strong>: %2$s</p></div>',
					esc_html( __( "ERROR", 'kiwi-contact-form' ) ),
					esc_html( __( "Failed to establish connection. Please double-check your configuration.", 'kiwi-contact-form' ) )
				);
				break;
			case 'updated':
				echo sprintf(
					'<div class="updated notice notice-success is-dismissible"><p>%s</p></div>',
					esc_html( __( "Configuration updated.", 'kiwi-contact-form' ) )
				);
				break;
		}
	}
	...
	public function display( $action = '' ) {
		echo '<p>' . sprintf(
			esc_html( __( 'The Constant Contact integration module allows you to send contact data collected through your contact forms to the Constant Contact API. You can create reliable email subscription services in a few easy steps. For details, see %s.', 'kiwi-contact-form' ) ),
			kiwi_cf_link(
				__(
					' ',
					'kiwi-contact-form'
				),
				__( 'Constant Contact Integration', 'kiwi-contact-form' )
			)
		) . '</p>';
		...
		if ( $this->is_active() ) {
			echo sprintf(
				...
				esc_html( __( "This site is connected to the Constant Contact API.", 'kiwi-contact-form' ) )
			);
		}
		...
		if ( 'setup' == $action ) {
			...
			echo sprintf(
				...
				esc_html( __( 'Setup Integration', 'kiwi-contact-form' ) )
			);
		}
	}
...
<?php wp_nonce_field( 'kiwi-constant-contact-setup' ); ?>
<table class="form-table">
<tbody>
<tr>
	<th scope="row"><label for="client_id"><?php echo esc_html( __( 'API Key', 'kiwi-contact-form' ) ); ?></label></th>
	<td><?php
		if ( $this->is_active() ) {
...
<tr>
	<th scope="row"><label for="client_secret"><?php echo esc_html( __( 'App Secret', 'kiwi-contact-form' ) ); ?></label></th>
	<td><?php
		if ( $this->is_active() ) {
			echo esc_html( kiwi_cf_mask_password( $this->client_secret ) );
			echo sprintf(
...
<tr>
	<th scope="row"><label for="redirect_uri"><?php echo esc_html( __( 'Redirect URI', 'kiwi-contact-form' ) ); ?></label></th>
	<td><?php
		echo sprintf(
	...
	?>
	<p class="description"><?php echo esc_html( __( "Set this URL as the redirect URI.", 'kiwi-contact-form' ) ); ?></p>
	</td>
</tr>
</tbody>
</table>
<?php
		if ( $this->is_active() ) {
			submit_button(
				_x( 'Reset Keys', 'API keys', 'kiwi-contact-form' ),
				'small', 'reset'
			);
		} else {
			submit_button(
				__( 'Connect to the Constant Contact API', 'kiwi-contact-form' )
			);
		}
?>
</form>
...
<?php wp_nonce_field( 'kiwi-constant-contact-edit' ); ?>
<table class="form-table">
<tbody>
<tr>
	<th scope="row"><?php echo esc_html( _x( 'Contact Lists', 'Constant Contact', 'kiwi-contact-form' ) ); ?></th>
	<td>
		<fieldset>
		<legend class="screen-reader-text"><span><?php echo esc_html( _x( "Contact Lists: Select lists to which newly added contacts are to belong.", 'Constant Contact', 'kiwi-contact-form' ) ); ?></span></legend>
		<p class="description"><?php echo esc_html( __( "Select lists to which newly added contacts are to belong.", 'kiwi-contact-form' ) ); ?></p>
		<ul class="checkboxes"><?php
			foreach ( $this->contact_lists as $list ) {
				echo sprintf(
					'<li><input %1$s /> <label for="%2$s">%3$s</label></li>',
					kiwi_cf_format_atts( array(
						'type' => 'checkbox',
						'name' => 'contact_lists[]',
						'value' => $list['list_id'],
...
class Kiwi_CF_ConstantContact_ContactPostRequest {
	...
	public function build( Kiwi_CF_Submission $submission ) {
		...
		$service_option = (array) kiwi::get_option( 'constant_contact' );
	...
	public function add_phone_number( $phone_number, $kind = 'home' ) {
		...
		or ! kiwi_cf_is_tel( $phone_number )
	...
	protected function strlen( $string ) {
		return kiwi_cf_count_code_units( stripslashes( $string ) );
	}
