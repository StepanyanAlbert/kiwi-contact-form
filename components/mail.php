...
class Kiwi_CF_Mail {
	...
	private function htmlize( $body ) {
		if ( $this->locale ) {
			$lang_atts = sprintf( ' %s',
				kiwi_cf_format_atts( array(
					'dir' => kiwi_cf_is_rtl( $this->locale ) ? 'rtl' : 'ltr',
		...
		$header = apply_filters( 'kiwi_cf_mail_html_header',
			'<!doctype html>
...
', $this );
		...
		$footer = apply_filters( 'kiwi_cf_mail_html_footer',
			'</body>
</html>', $this );
	...
	private function compose( $send = true ) {
		...
		$components = apply_filters( 'kiwi_cf_mail_components',
			$components, kiwi_cf_get_current_contact_form(), $this );

		if ( ! $send ) {
			return $components;
		}

		$subject = kiwi_cf_strip_newline( $components['subject'] );
		$sender = kiwi_cf_strip_newline( $components['sender'] );
		$recipient = kiwi_cf_strip_newline( $components['recipient'] );
		...
		if ( $this->use_html ) {
			...
			$headers .= "X-kiwi-Content-Type: text/html\n";
		} else {
			$headers .= "X-kiwi-Content-Type: text/plain\n";
		}
	...
	public function replace_tags( $content, $args = '' ) {
		...
		return kiwi_cf_mail_replace_tags( $content, $args );
	}
	...
	private function attachments( $template = null ) {
		...
		foreach ( explode( "\n", $template ) as $line ) {
			...
			if ( ! kiwi_cf_is_file_path_in_content_dir( $path ) ) {
...
function kiwi_cf_mail_replace_tags( $content, $args = '' ) {
	$args = wp_parse_args( $args, array(
		'html' => false,
		'exclude_blank' => false,
	) );
	...
	if ( is_array( $content ) ) {
		foreach ( $content as $key => $value ) {
			$content[$key] = kiwi_cf_mail_replace_tags( $value, $args );
		}
	...
	foreach ( $content as $num => $line ) {
		$line = new KiwiCfMailTaggedText( $line, $args );
...
add_action( 'phpmailer_init', 'kiwi_cf_phpmailer_init', 10, 1 );

function kiwi_cf_phpmailer_init( $phpmailer ) {
	$custom_headers = $phpmailer->getCustomHeaders();
	$phpmailer->clearCustomHeaders();
	$kiwi_cf_content_type = false;

	foreach ( (array) $custom_headers as $custom_header ) {
		$name = $custom_header[0];
		$value = $custom_header[1];

		if ( 'X-kiwi-Content-Type' === $name ) {
			$kiwi_cf_content_type = trim( $value );
		} else {
			$phpmailer->addCustomHeader( $name, $value );
		}
	}

	if ( 'text/html' === $kiwi_cf_content_type ) {
		$phpmailer->msgHTML( $phpmailer->Body );
	} elseif ( 'text/plain' === $kiwi_cf_content_type ) {
		$phpmailer->AltBody = '';
	}
}

class KiwiCfMailTaggedText {
	...
	private function replace_tags_callback( $matches, $html = false ) {
		...
		$mail_tag = new KiwiCfMailTag( $tag, $tagname, $values );
		...
		if ( null !== $submitted ) {
			...
			$replaced = kiwi_cf_flat_join( $replaced );
			...
			if ( $form_tag = $mail_tag->corresponding_form_tag() ) {
				...
				$replaced = apply_filters(
					"kiwi_cf_mail_tag_replaced_{$type}", $replaced,
					$submitted, $html, $mail_tag );
			}

			$replaced = apply_filters( 'kiwi_cf_mail_tag_replaced', $replaced,
				$submitted, $html, $mail_tag );
		...
		$special = apply_filters( 'kiwi_cf_special_mail_tags', null,
...
class KiwiCfMailTag {
	...
	public function __construct( $tag, $tagname, $values ) {
		...
		if ( ! empty( $values ) ) {
			...
			$this->values = kiwi_cf_strip_quote_deep( $matches[0] );
		}
	...
	public function corresponding_form_tag() {
		if ( $this->form_tag instanceof KiwiCfFormTag ) {
