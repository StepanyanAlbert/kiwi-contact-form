( function( $ ) {
	...
	$( 'form.tag-generator-panel .control-box :input' ).change( function() {
		...
		kiwi.taggen.normalize( $( this ) );
		kiwi.taggen.update( $form );
	} );

	$( 'input.insert-tag' ).click( function() {
		var $form = $( this ).closest( 'form.tag-generator-panel' );
		var tag = $form.find( 'input.tag' ).val();
		kiwi.taggen.insert( tag );
		tb_remove(); // close thickbox
		return false;
	} );

	kiwi.taggen.update = function( $form ) {
		...
		if ( $.isFunction( kiwi.taggen.update[ id ] ) ) {
			return kiwi.taggen.update[ id ].call( this, $form );
		}
		...
		$form.find( 'input.tag' ).each( function() {
			...
			var components = kiwi.taggen.compose( tag_type, $form );
			$( this ).val( components );
		} );
	...
	kiwi.taggen.update.captcha = function( $form ) {
		var captchac = kiwi.taggen.compose( 'captchac', $form );
		var captchar = kiwi.taggen.compose( 'captchar', $form );

		$form.find( 'input.tag' ).val( captchac + ' ' + captchar );
	};

	kiwi.taggen.compose = function( tagType, $form ) {
	...
	kiwi.taggen.normalize = function( $input ) {
		var val = $input.val();
		...
		if ( $input.is( ':checkbox.exclusive' ) ) {
			kiwi.taggen.exclusiveCheckbox( $input );
		}
	};

	kiwi.taggen.exclusiveCheckbox = function( $cb ) {
		if ( $cb.is( ':checked' ) ) {
			$cb.siblings( ':checkbox.exclusive' ).prop( 'checked', false );
		}
	};

	kiwi.taggen.insert = function( content ) {
		$( 'textarea#kiwi-form' ).each( function() {
			this.focus();
